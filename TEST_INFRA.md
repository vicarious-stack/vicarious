# E2E Test Infrastructure Design — Vicarious

This document outlines the testing philosophy, feature inventory, and detailed mapping of the 127 test cases for the Vicarious book and reader application.

---

## 1. Testing Philosophy

Due to the `CODE_ONLY` network isolation and interactive command-line permission constraints, standard browser automation tools (Selenium, Playwright) cannot be downloaded or executed in the validation environment. 

To achieve high-fidelity, comprehensive E2E validation, we utilize a **lightweight, pure-Python testing framework** (`e2e_tests/run_tests.py`) that operates on the following principles:

1. **Lightweight Pure-Python Static Parsing:**
   - Evaluates the structure and content of raw markdown files (`prologue.md`, `chapter_*.md`, `interlude_*.md`, `epilogue.md`, `appendices.md`).
   - Runs and validates the output of build scripts (`assemble.py`, `word_count.py`, `validate.py`).
2. **DOM Structure Auditing:**
   - Statically parses the HTML file (`index.html`) using Python's standard `html.parser` (or `xml.etree.ElementTree` / `BeautifulSoup` if available) to audit layout structures, containers, elements, loaded fonts, external scripts (like `marked.js`), and styles.
3. **JS Syntax & Logic Verification:**
   - Conducts static code audits, regular expression pattern matching, and AST-like parsing of the JavaScript block inside `index.html`.
   - Verifies the state management engine, `localStorage` key mappings, pagination boundaries, progress bar calculation logic, and custom markdown callout rules.

This approach guarantees robust verification of functional correctness, boundary limits, cross-feature state interactions, and real-world user scenarios, without requiring browser dependencies.

---

## 2. Feature Inventory (N = 11)

- **F1**: Content Presence & Layout (Markdown files, structural headers)
- **F2**: Book Assembly & Word Counts (Compilation pipeline, min word constraints)
- **F3**: Content Quality & Conventions (Sensory details, Digital Ledger, citations, diagrams)
- **F4**: Web Reader Theme Switcher (Midnight Void, Warm Alabaster, Amber Night themes)
- **F5**: Web Reader Typography & Header (Inter, Lora, Fira Code fonts, headers, metadata)
- **F6**: Web Reader Pagination (H1 splitting, Prev/Next controls, bounds validation)
- **F7**: Web Reader Table of Contents (Dynamic TOC generation, sidebar navigation, toggles)
- **F8**: Web Reader Text Controls (Font size increment, decrement, reset, bounds check)
- **F9**: Web Reader State Persistence (Theme, font size, section saving and restoring via localStorage)
- **F10**: Web Reader Custom Callout Parsing (Insight/Reality Check blockquote parsing and styles)
- **F11**: Web Reader Progress & Layout (Responsive layout, progress bar, transition animations, reading time)

---

## 3. Test Tiers

The test suite is structured into four progressive tiers to verify the application from low-level unit-like features to high-level integration scenarios:

- **Tier 1: Feature Coverage (55 tests)**: Verifies that every basic capability works under normal conditions.
- **Tier 2: Boundary & Corner Cases (55 tests)**: Tests limits, error-handling, empty inputs, corrupted states, off-offline behaviors, and extreme resolutions.
- **Tier 3: Cross-Feature Combinations (11 tests)**: Validates state interactions between multiple features (e.g., theme preservation on page transition).
- **Tier 4: Real-World Scenarios (6 tests)**: Simulates end-to-end user journeys (first-time user, persistent reading session, mobile session, CLI error validation).

---

## 4. Mapped Test Cases (127 total)
